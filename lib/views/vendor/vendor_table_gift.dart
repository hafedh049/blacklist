...
class VendorTableGiftState extends State<VendorTableGift> with RestorationMixin {
  ...
  final Map<String, bool> _gifts = <String, bool>{};

  final Map<String, Map<String, dynamic>> _vaults = <String, Map<String, dynamic>>{};
  ...
  Future<List<VendorProduct>> _load() async {
    final QuerySnapshot<Map<String, dynamic>> products = await FirebaseFirestore.instance.collection("products").where("storeID", isEqualTo: widget.storeID).get();
    for (final QueryDocumentSnapshot<Map<String, dynamic>> product in products.docs) {
      if (!_vaults.containsKey(product.get("categoryID"))) {
        final QuerySnapshot<Map<String, dynamic>> category = await FirebaseFirestore.instance.collection("categories").where('categoryID', isEqualTo: product.get("categoryID")).limit(1).get();
        _vaults[product.get("categoryID")] = <String, dynamic>{
          "vault": category.docs.first.get('gift'),
          "sells": (await FirebaseFirestore.instance.collection("sells").where('categoryID', isEqualTo: product.get("categoryID")).where('clientID', isEqualTo: widget.clientID).count().get()).count ?? 0,
        };
      }
    }

    return products.docs.map((QueryDocumentSnapshot<Map<String, dynamic>> e) => VendorProduct.fromJson(e.data())).toList();
  }
    ...
    for (final VendorProduct product in _products) {
      if (product.selected && int.parse(product.cartController.text) > 0) {
        for (int index = 1; index <= int.parse(product.cartController.text); index++) {
          bool cond = (_vaults[product.categoryID]!["sells"] + index) % _vaults[product.categoryID]!["vault"] != 0;
          _gifts["${product.productReference}-$index"] = !cond;
          if (cond) {
                ...
                Column(
                  ...
                  children: <Widget>[
                    ...
                    AnimatedButton(
                      ...
                      onPress: () {
                        if (_products.where((VendorProduct product_) => product_.selected && int.parse(product_.cartController.text) > 0).isNotEmpty) {
                                                  ...
                                                  List<Future<DocumentReference<Map<String, dynamic>>>>.generate(
                                                    ...
                                                    (int _) {
                                                      return FirebaseFirestore.instance.collection("sells").add(
                                                            product.toJson()
                                                              ..["newPrice"] = _gifts["${product.productReference}-${_ + 1}"]! ? 0 : product.newPrice
